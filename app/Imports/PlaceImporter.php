 ...
 */
class PlaceImporter implements ToModel, WithHeadingRow, WithChunkReading, WithEvents, ShouldQueue
{
    /**
    ...
    /**
     * A static cache to store parent location IDs.
    ...
    /**
     * A static cache to store place type IDs.
     * This avoids redundant database queries for the same place type name.
     *
     * @var array
     */
    ...
    public function model(array $row)
    {
        ...
        // Look up the location_type_id based on the 'type' column from the spreadsheet.
        $typeName = $row['type'];
        $typeId = null;
        if (!empty($typeName)) {
            // Check the local cache for the location type ID.
        ...
        return Place::updateOrCreate(
            ...
            [
                ...
                'official_note' => $row['official_note'],
                'note' => $row['note_by_checker'],
            ]
        );
    }
